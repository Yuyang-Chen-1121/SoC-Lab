`timescale 1 ns / 1 ps
	...
	// Next address after ARREADY indicates previous address acceptance  
	  always @(posedge M_AXI_ACLK)                                       
	  begin                                                                           
		if(M_AXI_ARESETN == 0)axi_araddr <= axi_araddr;      
		else if(init_txn_pulse == 1'b1) 
			begin  
			if(axi_araddr>=1920*1080*2-1) // 1080p resolution                                                     
	        	axi_araddr <= 'b0;   
			else
	...
	// a parameterized threshold
	 ...
	 // committed.                                                                                              
	  ...
	  begin                                                                                                     
	    ...
	    //else if (M_AXI_BVALID && axi_bready && (write_burst_counter == {(C_NO_BURSTS_REQ-1){1}}) && axi_wlast)
	    else if (M_AXI_RVALID && axi_rready && (read_index == C_M_AXI_BURST_LEN-1) && (read_burst_counter/*[C_NO_BURSTS_REQ]*/==15))
	      reads_done <= 1'b1;                                                                                   
	    else                                                                                                    
...
zynq_hdmi hdmi01 (
               ...
               .hdmi_sda(hdmi_sda),
			    ...
			    .wea(axi_rready&M_AXI_RVALID),
				.dina(M_AXI_RDATA), // Use M_AXI_RDATA as input data
